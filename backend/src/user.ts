...
export default function registerUserRoutes(app: Hono<any>, authMiddleware: any) {
  ...
  const user = (await (c.env as any).DB.prepare("SELECT * FROM users WHERE id = ?").bind(userPayload.userId).first()) as any;
      ...
      return c.json({ user: { id: user.id, email: user.email, firstName: user.first_name, lastName: user.last_name, phone: user.phone, isAdmin: user.is_admin, role: user.is_admin ? "admin" : "user" } });
    } catch (error) {
  ...
  const servicesByAppointment = await getAppointmentServices((c.env as any).DB, appointmentIds);
      const formattedAppointments = (appointments.results || []).map((apt: any) => ({ id: apt.id, date: apt.date, time: apt.time, status: apt.status, description: apt.description, createdAt: apt.created_at, services: servicesByAppointment[apt.id] || [] }));
  ...
  await (c.env as any).DB.prepare("UPDATE users SET first_name = ?, last_name = ?, email = ?, phone = ? WHERE id = ?").bind(firstName, lastName, email, phone || null, userPayload.userId).run();

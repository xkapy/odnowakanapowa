...
export default function registerAuthRoutes(app: Hono<any>) {
  ...
  const token = jwt.sign({ userId: newUser.id, email: newUser.email, isAdmin: newUser.is_admin }, (c.env as any).JWT_SECRET, { expiresIn: "7d" });
      ...
      return c.json({ success: true, message: "Konto utworzone pomyślnie", token, user: { id: newUser.id, email: newUser.email, firstName: newUser.first_name, lastName: newUser.last_name, phone: newUser.phone, isAdmin: newUser.is_admin, role: newUser.is_admin ? "admin" : "user" } });
    } catch (error) {
  ...
  const token = jwt.sign({ userId: user.id, email: user.email, isAdmin: user.is_admin }, (c.env as any).JWT_SECRET, { expiresIn: "7d" });
      ...
      return c.json({ success: true, message: "Zalogowano pomyślnie", token, user: { id: user.id, email: user.email, firstName: user.first_name, lastName: user.last_name, phone: user.phone, isAdmin: user.is_admin, role: user.is_admin ? "admin" : "user" } });
    } catch (error) {
